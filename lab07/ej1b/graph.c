...
static bool invrep(graph_t g) {
    bool valid = g!=NULL && g->costs!=NULL;
    vertex_t i = 0;
    while(i < g->max_vertexs && valid) {
        // La fila no es un puntero nulo y su elemento diagonal es 0
        valid = g->costs[i] != NULL;
        valid = valid && g->costs[i][i] == 0;
        ++i;
    }
    return valid;
}
