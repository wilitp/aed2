...
static bool invrep(graph_t g) {
    bool valid = g!=NULL;
    // La diagonal tiene que ser 0
    for(vertex_t i = 0; i < g->max_vertexs && valid; ++i) {
        valid = g->costs[g_index(i, i, g->max_vertexs)] == 0;
    }
    // TODO: ver si hay algo mas que checkear
    return valid;
}
...
graph_t graph_from_file(const char *file_path)
{
    ...
    for (unsigned int i = 0; i < tam; ++i) { // filas
        for (unsigned int j = 0; j < tam; ++j) { // elementos de la fila i
